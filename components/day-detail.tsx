      ...
      <Tabs defaultValue="logs" value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="logs" className="space-y-4">
          {isLoading ? (
            ...
            <p className="text-center py-4 text-muted-foreground">記録がありません</p>
          )}
          <Button
            variant="outline"
            className="w-full"
        ...
        <TabsContent value="plans" className="space-y-4">
          {isLoading ? (
            ...
            <div className="space-y-2">
              {plans.map((plan) => (
                <WorkoutPlanItem key={plan.id} plan={plan} onUpdate={fetchData} />
              ))}
            </div>
          ) : (
            <p className="text-center py-4 text-muted-foreground">予定がありません</p>
          )}
          <Button
            variant="outline"
            className="w-full"
